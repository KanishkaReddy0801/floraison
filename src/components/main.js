...
export default function Main() {
  const divRefs = useRef([useRef(), useRef()]); // Create separate refs for each div

  useEffect(() => {
    const handleScroll = () => {
      for (let i = 0; i < divRefs.current.length; i++) {
        const divRef = divRefs.current[i].current; // Access the current ref using .current
        if (divRef) {
          const rect = divRef.getBoundingClientRect();
          const isVisible = rect.top < window.innerHeight;

          if (isVisible) {
            divRef.classList.add('animate');
          }
        }
      }
    };
  ...
  return (
    <>
      <div className="main-containe1" ref={divRefs.current[0]}> {/* Use divRefs.current[0] */}
      ...
      <div className="main-containe1" ref={divRefs.current[1]}> {/* Use divRefs.current[1] */}
        <div className="main-image">
          <img src={main12} alt="" />
        </div>
        <div className="main-content">
